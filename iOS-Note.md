...
经验积累：
1.导入第三方库：
1.pod之后重新打开项目之后一定要clean后再build否则会找不到刚刚安装的库
2.因为很多三方库都是用OC写的故导入时需要新建一个桥接文件：bridging.h来引入第三方库中的.h头文件才可以进行使用:
1.新建一个名字为bridging.h的头文件
2.在配置文件中加入XXX/bridging.h路径：XXX-Build Settings-Swift Compiler - Code Generation-Objective-C Bridging Header-XXX/bridging.h
...
运行循环：RunLoop：每个线程都有一个RunLoop来负责事件监听【触摸、时钟、网络】
...
原子属性特点：多线程情况下：原子属性在同一时刻只能有一个线程能对该属性赋值操作，读取都可以：系统默认会给setter方法加一把锁：自旋锁：并不能保证线程安全
非原子属性特点：多个线程可以同时读取、赋值
...
运行时：1.可以在运行时给某各类增加属性
2.可以在运行时获取某个对象所有属性名称
3.交换方法

NSCache: 缓存机制：1.内存不足自动释放：需要判断空
2.内存安全
3.可以设置缓存（池）大小和数量
